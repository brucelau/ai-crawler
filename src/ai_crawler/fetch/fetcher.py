...
from ai_crawler.core.types import CrawlPolicy, CrawlTask, RenderType
...
class Fetcher:
    ...
    def _navigate_page(self, page, task: CrawlTask, strategy: CrawlPolicy):
        if strategy.use_interactive_search and getattr(task, "query", None):
            ...
            from ai_crawler.extraction.templates import template_store
    ...
    def _fetch_with_httpx(
        ...
        try:
            from ai_crawler.sites.endpoints_store import get_best_endpoint
    ...
    def _fetch_with_playwright(
        ...
        try:
            ...
            proxy_settings = utils.structured_proxy_settings(proxy_server)
    ...
    def _fetch_with_uc(
        ...
        try:
            ...
            from ai_crawler.browser.human.mouse import SeleniumMouseAdapter
            from ai_crawler.browser.operator import BrowserOperator
            page = SeleniumPageAdapter(driver)
            op = BrowserOperator(page, SeleniumMouseAdapter(driver))
            return content, 200, op
        except Exception as exc:
            salvaged_html = ""
            try:
                if driver is not None:
