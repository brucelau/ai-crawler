...
from ai_crawler.antidetect.handler import (
    AntiBotHandler, BlockAnalyzer, BlockDetectionContext, BlockType,
)
from ai_crawler.antidetect.fingerprinter import AntiBotFingerprinter
from ai_crawler.crawl.telemetry import extract_response_headers
from ai_crawler.extraction import build_axtree_semantic_confirmation
from ai_crawler.core.types import CrawlPolicy, CrawlTask, RenderType, PagePattern
...
class FetchEngineer:
    ...
    def fetch_api(self, url: str, params: dict | None = None) -> dict | None:
        """Fetch a JSON API endpoint through the proxy. Returns parsed JSON or None."""
        try:
        ...
        proxy_policy = build_policy(0)
        proxy = self.proxy_provider.proxy_url(proxy_policy) if self.proxy_provider else None
        proxies = {"http": proxy, "https": proxy} if proxy else None

        try:
            with requests.Session(proxies=proxies, impersonate="chrome131") as client:
                resp = client.get(url, params=params, timeout=15,
                    headers={"Accept": "application/json"})
                if resp.status_code == 200:
                    return resp.json()
                log.info("fetch_api_non_200", url=url[:100], status=resp.status_code)
        except Exception as exc:
            log.info("fetch_api_error", url=url[:100], error=str(exc))
        return None
    ...
    def execute(self, task: CrawlTask, strategy: CrawlPolicy) -> Attempt:
        ...
        # Unified analysis: replaces is_blocked + detect_waf + detect_block_reason + extract_block_signals
        analysis = BlockAnalyzer.analyze(
            html=html,
            status_code=status_code,
            headers=extract_response_headers(page, status_code or 0),
        ...
        if (
            ...
            # Re-analyze after proxy rotation
            if ip_rotation_count:
                context = self._build_detection_context(task, page)
                analysis = BlockAnalyzer.analyze(
                    html=html, status_code=status_code,
                    headers=extract_response_headers(page, status_code or 0),
        ...
        anti_bot_fingerprint = self.fingerprinter.infer(
            html, response_headers, status_code,
            block_type if blocked else BlockType.NONE,
            analysis.waf_type,
            analysis.reason,
        ).to_dict()
        ...
        return Attempt(
            ...
            waf_detected=analysis.waf_type,
            block_reason=analysis.reason,
    ...
    def _retry_with_proxy_rotation(
        ...
        for ip_retry in range(self.max_ip_retries):
            ...
            blocked, block_type = analysis.blocked, analysis.block_type
            if not blocked:
