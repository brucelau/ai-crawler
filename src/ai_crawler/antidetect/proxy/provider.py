...
from ai_crawler.core.types import CrawlPolicy, ProxyType
...
class ProxyProvider:
    ...
    def _get_thordata_manager(self, pt: ProxyType):
        ...
        from ai_crawler.antidetect.proxy.thordata import ThorDataManager
    ...
    def proxy_url(self, strategy: CrawlPolicy) -> str | None:
        ...
        if proxy:
            ...
            log.warning("proxy_verification_failed", proxy_host=proxy.split("@")[-1] if "@" in proxy else proxy)
            return proxy

    def rotate_proxy(self, strategy: CrawlPolicy) -> str | None:
