...
class CrawlPolicy:
    ...
    @classmethod
    def from_tier(cls, tier: int, **overrides) -> "CrawlPolicy":
    ...
    @classmethod
    def get_tier_strategies(cls, start_tier: int, end_tier: int = 9) -> list["CrawlPolicy"]:
...
class CrawlTask:
    ...
    strategies: list[CrawlPolicy] = field(default_factory=list)
    current_index: int = 0
    fail_count: int = 0
    ...
    def create(cls, url: str, site: str, use_auto_strategies: bool = True) -> "CrawlTask":
        ...
        if use_auto_strategies:
            ...
            strategies = CrawlPolicyGenerator.get_optimal_strategies(
                site, pattern.value, engine, top_n=10
            )
            if not strategies:
                strategies = [CrawlPolicy()]
        else:
    ...
    def create_fast(cls, url: str, site: str) -> "CrawlTask":
        ...
        strategies = CrawlPolicyGenerator.get_default_strategies(site, pattern.value)
    ...
    def create_from_tier(
        ...
        start_tier = 1
        strategies = CrawlPolicy.get_tier_strategies(start_tier, end_tier=9)
    ...
    def current_strategy(self) -> CrawlPolicy | None:
    ...
    def add_strategy_front(self, strategy: CrawlPolicy) -> None:
        if self.current_index > 0:
            self.strategies.insert(self.current_index, strategy)
        else:
            self.strategies.insert(0, strategy)

    def add_strategy_next(self, strategy: CrawlPolicy) -> None:
