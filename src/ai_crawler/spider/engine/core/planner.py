...
from ai_crawler.spider.runtime.task_context import TaskContext
...
class Planner:
    def __init__(self, initial_tier_selector=None, trace_store=None, strategy_mode="optimal"):
        ...
        self.policy_engine = CrawlPolicyScorer(CrawlPolicyStatsStore(trace_store))
    ...
    def resolve(self, task: CrawlTask) -> CrawlPolicy | None:
    ...
    def reprioritize_after_failure(
        ...
        candidates = [CrawlPolicyCandidate.from_strategy(task, strategy) for strategy in remaining]
    ...
    def _apply_llm_tier(self, task: CrawlTask, memory: SiteMemory) -> None:
        ...
        if cached_tier is None:
            ...
            task.strategies = CrawlPolicy.get_tier_strategies(llm_tier, end_tier=9)
        ...
        if cached_tier != task.metadata.get("start_tier"):
            ...
            task.strategies = CrawlPolicy.get_tier_strategies(cached_tier, end_tier=9)
    ...
    def _apply_policy_order(self, task: CrawlTask):
        candidates = [CrawlPolicyCandidate.from_strategy(task, strategy) for strategy in task.strategies]
