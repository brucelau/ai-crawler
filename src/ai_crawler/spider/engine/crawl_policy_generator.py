...
然后通过 CrawlPolicyScorer 基于历史 trace 打分排序，取 top-N 作为预置策略。
"""
...
from ai_crawler.spider.runtime.crawl import CrawlPolicy, ProxyType, RenderType, PagePattern
from ai_crawler.spider.engine.policy_engine import CrawlPolicyCandidate, CrawlPolicyScorer, CrawlPolicyStatsStore


class CrawlPolicyGenerator:
    """自动策略生成器"""
    ...
    @classmethod
    def _candidate_to_strategy(cls, candidate: CrawlPolicyCandidate) -> CrawlPolicy:
        """将 CrawlPolicyCandidate 转换为 CrawlPolicy"""
        proxy_map = {
    ...
    def get_optimal_strategies(
        ...
        site: str,
        page_pattern: str,
        policy_engine: CrawlPolicyScorer,
        top_n: int = 10,
    ) -> list[CrawlPolicy]:
        """基于 CrawlPolicyScorer 历史数据获取最优策略顺序"""
        ...
        fake_task = _FakeTask()
        policy_candidates = [
            CrawlPolicyCandidate.from_strategy(fake_task, c, source="auto")
            for c in candidates
        ]
    ...
    def get_default_strategies(
        ...
        def sort_key(s: CrawlPolicy) -> tuple[int, int]:
...
def policy_candidate_to_strategy(candidate: CrawlPolicyCandidate) -> CrawlPolicy:
    """将 CrawlPolicyCandidate 转换为 CrawlPolicy 的便捷函数"""
    return CrawlPolicyGenerator._candidate_to_strategy(candidate)
