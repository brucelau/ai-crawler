"""Intexcorp site adapter — search command."""
...
@register(site="intexcorp", command="search")
class IntexcorpSearch(Command):
    site = "intexcorp"
    command = "search"
    url_template = "https://intexcorp.com/search?q={query}"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ['.product-tile', '.product-card', '.product']:
        ...
        for item in items:
            # Title
            title = ""
            for sel in ['.product-title', 'h3', '.product-name']:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Intexcorp product page."""
        soup = BeautifulSoup(html, "html.parser")
