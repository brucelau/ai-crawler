"""Etsy site adapter — search command."""
...
@register(site="etsy", command="search")
class EtsySearch(Command):
    site = "etsy"
    command = "search"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ['[data-search-results] li', '.listing-card', '.v2-listing-card']:
        ...
        for item in items:
            # Title
            title = ""
            for sel in ['.v2-listing-card__title', '.listing-card__title', 'h3']:
            ...
            # Price
            price = ""
            for sel in ['.currency-value', '.lc-price .currency-value', '.n-listing-card__price']:
                el = item.select_one(sel)
                if el:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Etsy product page."""
        soup = BeautifulSoup(html, "html.parser")
