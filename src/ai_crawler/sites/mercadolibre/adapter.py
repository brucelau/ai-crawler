"""Mercadolibre site adapter — search command."""
...
@register(site="mercadolibre", command="search")
class MercadolibreSearch(Command):
    site = "mercadolibre"
    command = "search"
    url_template = "https://listado.mercadolibre.com/{query}"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ['.ui-search-result', '.andes-card', 'li.ui-search-layout__item']:
        ...
        for item in items:
            # Title
            title = ""
            for sel in ['.ui-search-item__title', 'h2']:
            ...
            # Price
            price = ""
            for sel in ['.price-tag-fraction', '.andes-money-amount__fraction', '.ui-search-price__second-line']:
                el = item.select_one(sel)
                if el:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Mercadolibre product page."""
        soup = BeautifulSoup(html, "html.parser")
