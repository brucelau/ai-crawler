"""Action site adapter — search command."""
...
@register(site="action", command="search")
class ActionSearch(Command):
    site = "action"
    command = "search"
    url_template = "https://www.action.com/en/search/?q={query}"
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Action product page."""
        soup = BeautifulSoup(html, "html.parser")
