"""Menards site adapter — search command."""
...
@register(site="menards", command="search")
class MenardsSearch(Command):
    site = "menards"
    command = "search"
    url_template = "https://www.menards.com/main/search.html?search={query}"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ['.product-card', '.search-result', '.product-tile']:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Menards product page."""
        soup = BeautifulSoup(html, "html.parser")
