"""Kohls site adapter — search command."""
...
@register(site="kohls", command="search")
class KohlsSearch(Command):
    site = "kohls"
    command = "search"
    url_template = "https://www.kohls.com/search.jsp?search={query}"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ['.product-card', "[data-testid='product-card']", '.products-grid li']:
        ...
        for item in items:
            # Title
            title = ""
            for sel in ['.product-title', '.prod_name', 'h3']:
            ...
            # Price
            price = ""
            for sel in ['.product-price', '.prod_price', '.price']:
                el = item.select_one(sel)
                if el:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Kohls product page."""
        soup = BeautifulSoup(html, "html.parser")
