"""Walmartmexico site adapter — search command."""
...
@register(site="walmartmexico", command="search")
class WalmartmexicoSearch(Command):
    site = "walmartmexico"
    command = "search"
    url_template = "https://www.walmart.com.mx/busca?q={query}"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ['.product-card', "[data-testid='product-card']"]:
        ...
        for item in items:
            ...
            # Price
            price = ""
            for sel in ['.price', '.product-price']:
                el = item.select_one(sel)
                if el:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Walmartmexico product page."""
        soup = BeautifulSoup(html, "html.parser")
