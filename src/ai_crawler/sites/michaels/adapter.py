"""Michaels site adapter — search command."""
...
@register(site="michaels", command="search")
class MichaelsSearch(Command):
    site = "michaels"
    command = "search"
    url_template = "https://www.michaels.com/search?q={query}"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ['.product-tile', "[data-testid='product-tile']"]:
        ...
        for item in items:
            # Title
            title = ""
            for sel in ['.product-title', '.name', 'h3']:
            ...
            # Price
            price = ""
            for sel in ['.product-price', '.price', '.regular-price']:
                el = item.select_one(sel)
                if el:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Michaels product page."""
        soup = BeautifulSoup(html, "html.parser")
