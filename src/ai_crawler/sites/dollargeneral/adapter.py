"""Dollargeneral site adapter — search command."""
...
@register(site="dollargeneral", command="search")
class DollargeneralSearch(Command):
    site = "dollargeneral"
    command = "search"
    url_template = "https://www.dollargeneral.com/search?q={query}"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ['.product-tile', '.product-card', '.search-result-item']:
        ...
        for item in items:
            # Title
            title = ""
            for sel in ['.product-name', 'h3', '.title']:
            ...
            # Price
            price = ""
            for sel in ['.price', '.sale-price', '.product-price']:
                el = item.select_one(sel)
                if el:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Dollargeneral product page."""
        soup = BeautifulSoup(html, "html.parser")
