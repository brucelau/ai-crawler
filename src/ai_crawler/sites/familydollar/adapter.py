"""Familydollar site adapter — search command."""
...
@register(site="familydollar", command="search")
class FamilydollarSearch(Command):
    site = "familydollar"
    command = "search"
    url_template = "https://www.familydollar.com/search?q={query}"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ['.product-tile', '.product-card']:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Familydollar product page."""
        soup = BeautifulSoup(html, "html.parser")
