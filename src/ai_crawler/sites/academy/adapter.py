"""Academy site adapter — search command."""
...
@register(site="academy", command="search")
class AcademySearch(Command):
    site = "academy"
    command = "search"
    url_template = "https://www.academy.com/search?q={query}"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ['.product-tile', '.product-card', "[data-testid='product-card']"]:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Academy product page."""
        soup = BeautifulSoup(html, "html.parser")
