"""Lowes site adapter — search command."""
...
@register(site="lowes", command="search")
class LowesSearch(Command):
    site = "lowes"
    command = "search"
    url_template = "https://www.lowes.com/search?searchTerm={query}"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ["[data-testid='product-card']", '.product-card', '.grid-item']:
        ...
        for item in items:
            # Title
            title = ""
            for sel in ['.product-title', 'h3', '.description']:
            ...
            # Link
            link = ""
            for sel in ["a[href*='/pd/']", "a[href*='/product/']"]:
            ...
            # Price
            price = ""
            for sel in ['.product-price', '.price', "[data-testid='price']"]:
                el = item.select_one(sel)
                if el:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Lowes product page."""
        soup = BeautifulSoup(html, "html.parser")
