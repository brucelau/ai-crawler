"""Samsclub site adapter — search command."""
...
@register(site="samsclub", command="search")
class SamsclubSearch(Command):
    site = "samsclub"
    command = "search"
    url_template = "https://www.samsclub.com/s/{query}"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ["[data-testid='product-card']", '.sc-product-card']:
        ...
        for item in items:
            # Title
            title = ""
            for sel in ['.sc-product-title', 'h4', '.product-title']:
            ...
            # Link
            link = ""
            for sel in ["a[href*='/p/']", "a[href*='/prod/']"]:
            ...
            # Price
            price = ""
            for sel in ['.sc-price', '.price', '.product-price']:
                el = item.select_one(sel)
                if el:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Samsclub product page."""
        soup = BeautifulSoup(html, "html.parser")
