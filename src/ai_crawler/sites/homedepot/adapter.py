"""Homedepot site adapter — search command."""
...
@register(site="homedepot", command="search")
class HomedepotSearch(Command):
    site = "homedepot"
    command = "search"
    url_template = "https://www.homedepot.com/s/{query}"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ["[data-testid='product-pod']", '.product-pod', '.grid-item']:
        ...
        for item in items:
            # Title
            title = ""
            for sel in ['.product-title', '.product-pod__title', 'h3']:
            ...
            # Link
            link = ""
            for sel in ["a[href*='/p/']", "a[href*='/product/']"]:
            ...
            # Price
            price = ""
            for sel in ['.price', '.product-pod__price', "[data-testid='price']"]:
                el = item.select_one(sel)
                if el:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Homedepot product page."""
        soup = BeautifulSoup(html, "html.parser")
