"""Bunnings site adapter — search command."""
...
@register(site="bunnings", command="search")
class BunningsSearch(Command):
    site = "bunnings"
    command = "search"
    url_template = "https://www.bunnings.com.au/search/products?q={query}"
    ...
    def extract(self, html: str, url: str) -> list[Product]:
        ...
        items = []
        for sel in ['.product-tile', '.product-card', '.search-result']:
    ...
    def extract_detail(self, html: str, url: str) -> dict:
        """Extract product detail fields from a Bunnings product page."""
        soup = BeautifulSoup(html, "html.parser")
